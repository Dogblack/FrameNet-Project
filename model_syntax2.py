...
class Encoder(nn.Module):
    ...
    def forward(self, word_input: torch.Tensor, lemma_input: torch.Tensor, pos_input: torch.Tensor,
                head_input:torch.Tensor, rel_input:torch.Tensor,
                lengths:torch.Tensor, frame_idx, token_type_ids=None, attention_mask=None):
        ...
        attentional_target_state = local_attention(attention_mask=attention_mask, hidden_state=output,
                                                   frame_idx=frame_idx,target_state=target_state,
...
class Model(nn.Module):
    ...
    def forward(self, word_ids, lemma_ids, pos_ids, head_ids, rel_ids, lengths, frame_idx, fe_mask=None, lu_mask=None,
                frame_len=None, token_type_ids=None, attention_mask=None):
        encoder_output, encoder_state, target_state = self.encoder(word_input=word_ids, lemma_input=lemma_ids,
                                                                   pos_input=pos_ids, head_input=head_ids,
                                                                   rel_input=rel_ids, lengths=lengths,
                                                                   frame_idx=frame_idx,
