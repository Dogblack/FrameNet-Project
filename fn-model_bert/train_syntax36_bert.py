...
from dataset_bert import FrameNetDataset
from torch.utils.data import DataLoader
from utils import get_mask_from_index,seed_everything
from evaluate10 import Eval
from config_bert import get_opt
from data_process_bert import DataConfig
from model_syntax36_with_bert import Model
...
def evaluate(opt, model, dataset, best_metrics=None, show_case=False):
    ...
    with torch.no_grad():
        ...
        for batch in test_dl:
            word_ids,  lengths, attention_mask, target_head, target_tail, \
            target_type, fe_head, fe_tail, fe_type, fe_cnt, \
            fe_cnt_with_padding, fe_mask, lu_mask, token_type_ids,sent_length,target_mask_ids = batch

            return_dic = model(word_ids=word_ids, lengths=lengths,
                               frame_idx=(target_head, target_tail),
                               token_type_ids=token_type_ids,
                               attention_mask=attention_mask, fe_mask=fe_mask, lu_mask=lu_mask,target_mask_ids=target_mask_ids)
...
if __name__ == '__main__':
    ...
    num_train_steps = int((len(train_dataset) / opt.batch_size) * opt.epochs)
    # prepare optimizer
    ...
    if opt.mode == 'train':
        for epoch in range(1, epochs):
            ...
            for batch in train_dl:
                ...
                return_dic = model(word_ids=word_ids
                                  ,lengths=lengths,
                                   frame_idx=(target_head, target_tail),
                                   token_type_ids=token_type_ids,
                                   attention_mask=attention_mask, fe_mask=fe_mask, lu_mask=lu_mask,target_mask_ids=target_mask_ids)
