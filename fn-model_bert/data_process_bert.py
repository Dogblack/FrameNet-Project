...
from config_bert import get_opt,PLMConfig
...
def instance_process(lines, maxlen):
    ...
    for line in lines:
        if line[0:3] == '# i':
            ...
            word_list = bert_tokenizer.tokenize(sent)

            instance_dic[(sent_id, target_type, cnt)]['length'] = min(len(word_list),maxlen)

            attention_mask = [0]*maxlen
            attention_mask[0:min(len(word_list),maxlen)]=[1] * min(len(word_list),maxlen)

            instance_dic[(sent_id, target_type, cnt)]['attention_mask']=attention_mask

            word_list_padded=padding_sentence(word_list, maxlen)
            instance_dic[(sent_id, target_type, cnt)]['word_list'] = word_list_padded
            instance_dic[(sent_id, target_type, cnt)]['tokenized_ids'] = bert_tokenizer.convert_tokens_to_ids(word_list_padded)
            ...
            target_start_list=bert_tokenizer.tokenize(target_start_token)
            target_end_list = bert_tokenizer.tokenize(target_end_token)
            # print(target_start_list)
            # print(target_end_list)
            # print(target_start_list[0])
            # print(target_end_list[-1])
            # print(word_list)
            target_start=word_list.index(target_start_list[0],int(target_idx[0]))
            target_end=word_list.index(target_end_list[-1],int(target_idx[1]))
            instance_dic[(sent_id, target_type, cnt)]['target_idx'] = (target_start,target_end)

            instance_dic[(sent_id, target_type, cnt)]['span_start']=[]
            instance_dic[(sent_id, target_type, cnt)]['span_end']=[]
            for i in range(len(span_start)):
                span_start_list = bert_tokenizer.tokenize(span_start_token[i])
                span_end_list = bert_tokenizer.tokenize(span_end_token[i])
                span_start_idx = word_list.index(span_start_list[0], span_start[i])
                span_end_idx = word_list.index(span_end_list[-1], span_end[i])


                instance_dic[(sent_id, target_type, cnt)]['span_start'].append(span_start_idx)
                instance_dic[(sent_id, target_type, cnt)]['span_end'].append(span_end_idx)
            ...
            if data_list[12] != '_' and data_list[13] != '_':
                ...
                if target_idx == [-1, -1]:
                    ...
                    target_start_token = data_list[1]
                    target_end_token = data_list[1]
                else:
            ...
            if data_list[14] != '_':
                ...
                if fe[0] == 'B' and find is False:
                    ...
                    span_start_token.append(data_list[1])
                    find = True

                elif fe[0] == 'O':
                    span_end.append(int(data_list[0]) - 1)
                    span_end_token.append(data_list[1])
